...
vk::Extent2D
Swapchain::querySurfaceExtent(const vk::SurfaceCapabilitiesKHR& capability,
                              int windowWidth, int windowHeight) {
    if (capability.currentExtent.width !=
        ...
        extent.height =
            std::clamp(extent.height, capability.minImageExtent.height,
...
vk::SwapchainKHR Swapchain::createSwapchain() {
    ...
    auto& ctx = Context::Instance();
    if (ctx.queueInfo.graphicsIndex.value() ==
        ctx.queueInfo.presentIndex.value()) {
        createInfo.setImageSharingMode(vk::SharingMode::eExclusive);
    } else {
        createInfo.setImageSharingMode(vk::SharingMode::eConcurrent);
        std::array queueIndices = {ctx.queueInfo.graphicsIndex.value(),
                                   ctx.queueInfo.presentIndex.value()};
...
void Swapchain::createFramebuffers() {
    for (auto& img : images) {
        ...
        framebuffers.push_back(
            Context::Instance().device.createFramebuffer(createInfo));
    }
}
