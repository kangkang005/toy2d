...
Swapchain::Swapchain(int w, int h) {
  ...
  if (queueIndicecs.graphicsQueue.value() ==
...
void Swapchain::queryInfo(int w, int h) {
  ...
  // if info.maxImageCount == 0, it means that there is no maximum
  // imageCount must be greater than or equal to mimImageCount
  ...
  info.imageExtent.width = std::clamp<uint32_t>(
      w, capabilities.minImageExtent.width, capabilities.minImageExtent.width);
...
void Swapchain::createImageViews() {
  ...
  for (int i = 0; i < images.size(); i++) {
    ...
    createInfo.setImage(images[i])
        ...
        .setFormat(info.format.format) // same as image of swapchain
