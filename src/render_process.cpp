...
void RenderProcess::RecreateGraphicsPipeline(
    ...
    const std::vector<char>& fragSource) {
    if (graphicsPipeline) {
...
vk::PipelineLayout RenderProcess::createLayout() {
    ...
    createInfo.setPushConstantRangeCount(0).setSetLayoutCount(0);

    return Context::Instance().device.createPipelineLayout(createInfo);
}

vk::Pipeline
RenderProcess::createGraphicsPipeline(const std::vector<char>& vertexSource,
                                      const std::vector<char>& fragSource) {
    auto& ctx = Context::Instance();
    ...
    inputAsmCreateInfo.setPrimitiveRestartEnable(false).setTopology(
        vk::PrimitiveTopology::eTriangleList);

    // 3. viewport and scissor
    vk::PipelineViewportStateCreateInfo viewportInfo;
    vk::Viewport viewport(0, 0, ctx.swapchain->GetExtent().width,
                          ctx.swapchain->GetExtent().height, 0, 1);
    ...
    multisampleInfo.setSampleShadingEnable(false).setRasterizationSamples(
    ...
    blendAttachmentState.setBlendEnable(false).setColorWriteMask(
    ...
    blendInfo.setAttachments(blendAttachmentState).setLogicOpEnable(false);
    ...
    if (result.result != vk::Result::eSuccess) {
        std::cout << "create graphics pipeline failed: " << result.result
                  << std::endl;
    }
...
vk::RenderPass RenderProcess::createRenderPass() {
    ...
    // executable dependency between subpass
    //
    // set sync dependency when enter to renderPass if set SrcSubpass as
    // VK_SUBPASS_EXTERNAL
    vk::SubpassDependency dependency;
    dependency.setSrcSubpass(VK_SUBPASS_EXTERNAL)
        .setDstSubpass(0)
        // wait for swapchain to finish reading from the image before can access
        // it
    ...
    // If initialLayout does not match the layout of the attachment reference in
    // the first subpass, there will be an implicit transition before starting
    // the render pass.
    //
    // If finalLayout does not match the layout of the attachment reference in
    // the last subpass, there will be an implicit transition at the end.
    ...
    reference.setAttachment(0).setLayout(
        vk::ImageLayout::eColorAttachmentOptimal);

    vk::SubpassDescription subpassDesc;
    subpassDesc.setColorAttachments(reference).setPipelineBindPoint(
