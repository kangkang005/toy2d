...
void RenderProcess::InitPipeline(int width, int height) {
  ...
  blend.setLogicOpEnable(false).setAttachments(attachs);
  ...
  auto result =
      Context::GetInstance().device.createGraphicsPipeline(nullptr, createInfo);
...
void RenderProcess::InitRenderPass() {
  ...
  // relationship between framebuffer with attachment:
  //
  // framebuffer <- attachment group
  // 1. color attachment+ -> image
  // 2. stencil & depth*  -> image
  ...
  // InitSubpass -> Subpass1 -> Subpass2 .. -> Subpass[n]
