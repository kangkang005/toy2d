...
void Context::createInstance(const std::vector<const char *> &extensions) {
  ...
  createInfo.setPEnabledLayerNames(layers).setPEnabledExtensionNames(
      extensions);
...
void Context::pickupPhyiscalDevice() {
  ...
  std::cout << "Pick up phyical device:" << std::endl;
...
// queue family meanwhile supports graphics and present
